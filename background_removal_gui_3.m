...
function varargout = background_removal_gui_3(varargin)
% BACKGROUND_REMOVAL_GUI_3 MATLAB code for background_removal_gui_3.fig
%      BACKGROUND_REMOVAL_GUI_3, by itself, creates a new BACKGROUND_REMOVAL_GUI_3 or raises the existing
%      singleton*.
%
%      H = BACKGROUND_REMOVAL_GUI_3 returns the handle to a new BACKGROUND_REMOVAL_GUI_3 or the handle to
%      the existing singleton*.
%
%      BACKGROUND_REMOVAL_GUI_3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in BACKGROUND_REMOVAL_GUI_3.M with the given input arguments.
%
%      BACKGROUND_REMOVAL_GUI_3('Property','value',...) creates a new BACKGROUND_REMOVAL_GUI_3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before background_removal_gui_3_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to background_removal_gui_3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help background_removal_gui_3

% Last Modified by GUIDE v2.5 07-Aug-2019 10:22:19

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @background_removal_gui_3_OpeningFcn, ...
                   'gui_OutputFcn',  @background_removal_gui_3_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before background_removal_gui_3 is made visible.
function background_removal_gui_3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to background_removal_gui_3 (see VARARGIN)
...
% Choose default command line output for background_removal_gui_3
...
% UIWAIT mak es background_removal_gui_3 wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = background_removal_gui_3_OutputFcn(hObject, eventdata, handles) 
...
function add_point_Callback(hObject, eventdata, handles)
    ...
    prep_channel_params(handles);
    
function prep_channel_params(handles)
    global pipeline_data
    labels = pipeline_data.points.labels();
    params_string = cell(size(labels));
    if ~isfield(pipeline_data, 'thresholds')
        pipeline_data.thresholds = zeros(size(pipeline_data.points.labels()))+0.2;
    end
    for i=1:numel(labels)
        params_string{i} = tabJoin({labels{i}, num2str(pipeline_data.thresholds(i))}, 20);
    end
    set(handles.channel_params_listbox, 'string', params_string);
...
function threshold_bkg_Callback(hObject, eventdata, handles)
    try
        if isnan(str2double(get(hObject,'string')))
            ...
            channel_idxs = get(handles.channel_params_listbox, 'value');
            global pipeline_data;
            for i=1:numel(channel_idxs)
                channel_idx = channel_idxs(i);
                pipeline_data.thresholds(channel_idx) = str2double(get(hObject, 'string'));
                prep_channel_params(handles);
            end
        end
    catch
...
function test_Callback(hObject, eventdata, handles)
    % try
        ...
        drawnow
        MIBItestBackgroundParameters_3(get(handles.radiobutton2, 'value'));
...
function evaluate_point_Callback(hObject, eventdata, handles)
    % try
        ...
        set(handles.figure1, 'pointer', 'watch');
        drawnow;
        MIBIevaluateBackgroundParameters_3({evalPoint});
        set(handles.figure1, 'pointer', 'arrow');
...
function evaluate_all_points_Callback(hObject, eventdata, handles)
    try
        ...
        set(handles.figure1, 'pointer', 'watch');
        drawnow
        MIBIevaluateBackgroundParameters_3(evalPoints);
        set(handles.figure1, 'pointer', 'arrow');
...
% --- Executes on selection change in channel_params_listbox.
function channel_params_listbox_Callback(hObject, eventdata, handles)
% hObject    handle to channel_params_listbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    channel_idx = get(hObject, 'value');
    if numel(channel_idx)==1
        global pipeline_data;
        t = num2str(pipeline_data.thresholds(channel_idx));
        set(handles.threshold_bkg, 'string', t);
        pipeline_data.t_index = channel_idx;
    end
% Hints: contents = cellstr(get(hObject,'String')) returns channel_params_listbox contents as cell array
%        contents{get(hObject,'Value')} returns selected item from channel_params_listbox


% --- Executes during object creation, after setting all properties.
function channel_params_listbox_CreateFcn(hObject, eventdata, handles)
% hObject    handle to channel_params_listbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
