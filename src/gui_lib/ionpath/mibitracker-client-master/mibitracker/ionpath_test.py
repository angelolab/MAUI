...
class MibiRequests(object):
    ...
    def __init__(self, url, token, retries=MAX_RETRIES,
...
# mibreq = MibiRequests('https://backend-dot-mibitracker-angelolab.appspot.com', 'alex.baranski@gmail.com', '9g4VzcEb}dvwTa+iLPqaHu%QoMJ9D@7g')
import sys
# argv[1] will be -auth, argv[2] will be url, argv[3] will be auth_token
mibreq = MibiRequests(sys.argv[2], sys.argv[3])
if len(sys.argv)==4:
    temp = mibreq.get('/runs/')
if len(sys.argv)>4:
    if sys.argv[4]=='-get':
        temp = mibreq.get(sys.argv[5])
        data = temp.json()
        print(data)
    elif sys.argv[4]=='-upload':
...
# if len(sys.argv)==4 and sys.argv[1]=='-auth':
#     # arg[2] will be url, arg[3] will be an auth_token
#     mibreq = MibiRequests(sys.argv[2], sys.argv[3])
#     temp = mibreq.get('/runs/')
# if len(sys.argv)==6 and sys.argv[1]=='-auth':
#     # arg[2] will be url, arg[3] will be an auth_token
#     mibreq = MibiRequests(sys.argv[2], sys.argv[3])
#     # arg[4] will be either '-get', '-post', or '-put', arg[5] will be an argument
#     if sys.argv[4]=='-get':
#         temp = mibreq.get(sys.argv[5])
#         data = temp.json()
#         print(data)
...
# shir = data[0]
# run = shir['name']
# folder = shir['imageset']['images'][0]['folder']
#
#
# filepath = '/Volumes/G-DRIVE mobile USB/bangelo_data/decidua181105test/extracted/Point1.tiff'
#
# print(folder)